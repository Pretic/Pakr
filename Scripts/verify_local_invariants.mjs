...
assert.match(indexHtml, /<title>PakrPre<\/title>/, 'local frontend should keep the deployed PakrPre branding');
assert.match(indexHtml, /github\.com\/Pretic\/Pakr/, 'project link should target the maintained repository');
assert.match(indexHtml, /ZhangShengFan\/Pakr main/, 'sync panel should name the canonical upstream');
assert.doesNotMatch(indexHtml, /github\.com\/Pretic\/PakrPre/, 'frontend should not link to the deleted repository');
...
assert.match(worker, /return 'ZhangShengFan\/Pakr'/, 'sync upstream repo should be pinned');
assert.match(
  worker,
...
assert.match(syncWorkflow, /ZhangShengFan\/Pakr/, 'sync workflow should allow only the canonical upstream');
...
console.log('Local invariant checks passed');
